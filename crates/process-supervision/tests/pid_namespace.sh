...
#
# This is the only launcher for the ignored process-tree namespace probe.  The
...
exec "$unshare_path" --user --map-root-user --pid --fork --mount-proc sh -eu -c '
    ...
    exec cargo test --locked -p jmeter-rs-process-supervision --lib process_tree_cleanup_is_namespace_scoped -- --exact --ignored --nocapture
'
