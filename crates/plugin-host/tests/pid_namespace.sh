...
#
# This is the only launcher for the ignored plugin-host process-group test.
# The inner shell proves the user/PID namespace, PID 1, and namespace-local
# /proc before inheriting a proof token into the one exact Cargo test.  It has
# no cleanup or signal path of its own.
...
export PLUGIN_HOST_HOST_PID_NAMESPACE="$host_pid_namespace"
exec "$unshare_path" --user --map-root-user --pid --fork --mount-proc sh -eu -c '
    ...
    exec cargo test --locked -p jmeter-rs-plugin-host --test supervisor process_group_cleanup_is_namespace_scoped -- --exact --ignored --nocapture
'
