...
#
# This is the only launcher for the ignored Java-bridge process-group test.
...
export JBR_HOST_PID_NAMESPACE="$host_pid_namespace"
exec "$unshare_path" --user --map-root-user --pid --fork --mount-proc sh -eu -c '
    ...
    exec cargo test --locked -p jmeter-rs-java-bridge --test supervisor process_group_cleanup_is_namespace_scoped -- --exact --ignored --nocapture
'
